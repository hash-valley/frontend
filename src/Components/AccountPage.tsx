...
import { GridContainer, GridItem, Page, SuccessText, Spaced } from "../Styles/Components"
...
const AccountPage = () => {
  ...
  const [mults, setMults] = useState<Mults>({
    canWater: 0, canPlant: 0, canHarvest: 0
  })

  const { loading, error, data, refetch } = useQuery(ACCOUNT_QUERY, { variables: { userAddress: userAddress.toLowerCase() } })
            ...
            <div>
              {
                userAddress === wallet.account ?
                  <h3>
                    ...
                    {data.account.bottles.length === 1 ? "" : "s"} and {formatNum(utils.formatEther(data.account.vinegarBalance))} Vinegar
                  </h3>
                  :
                  <h3>
              ...
              <div>
                {
                  view == "bottles" &&
                  <div>
                    <Spaced primary onClick={() => history.push(`/account/${userAddress}/vineyards`)}>
                      Vineyards
                    </Spaced>
                    <Spaced primary reverse active>Bottles</Spaced>
                  </div>
                }
                {
                  (view == "vineyards" || view == undefined) &&
                  <div>
                    <Spaced primary reverse active>Vineyards</Spaced>
                    <Spaced primary onClick={() => history.push(`/account/${userAddress}/bottles`)}>
                      Bottles
                    </Spaced>
                  </div>
                }
              ...
              <div>
                {
                  wallet.status === "connected" && userAddress === wallet.account ? <div>
                    {
                      mults.canPlant > 0 ?
                        <Spaced primary onClick={sendPlantMultiple}>Plant All</Spaced>
                        : null
                    }
                    {
                      mults.canWater > 0 ?
                        <Spaced primary onClick={sendWaterMultiple}>Water All</Spaced>
                        : null
                    }
                    {
                      mults.canHarvest > 0 ?
                        <Spaced primary onClick={sendHarvestMultiple}>Harvest All</Spaced>
                        : null
                    }
                  </div> : null
                }
